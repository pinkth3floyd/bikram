...
export const FaceEnrollment: React.FC<FaceEnrollmentProps> = ({
  ...
  const { faceVerified, faceId, faceVerificationEnabled, isLoading: statusLoading } = useFaceVerificationStatus();
  ...
  const handleFaceAlreadyEnrolled = () => {
    const errorMsg = 'Face already enrolled. You can proceed to verification or contact support if you need to re-enroll.';
    setError(errorMsg);
    onError?.(errorMsg);
  };
  ...
  async function enrollNewUser() {
    if (!faceioRef.current) {
      const errorMsg = 'Face recognition module not initialized. Please refresh the page.';
      setError(errorMsg);
      onError?.(errorMsg);
      return;
    }
  ...
  if (error) {
    const isAlreadyEnrolled = error.includes('already enrolled') || error.includes('Face not enrolled');
  ...
  return (
    <div className={`space-y-4 ${className}`}>
      ...
      {faceVerified && (
        <div className='text-center'>
          <div className='text-green-600 text-2xl mb-2'>✓</div>
          <h3 className='text-lg font-semibold text-green-600'>Face Verification Complete</h3>
        </div>
      )}

      {/* Instructions */}
      <div className='mt-4 p-3 bg-yellow-50 border border-yellow-200 rounded-lg'>
        <h4 className='font-semibold text-yellow-800 mb-2 text-sm'>Instructions:</h4>
